...
#对AST节点进行截取，只截取顶层节点，还原成源码并返回所在行数
...
#对astroid节点进行截取，只保留最上层节点，还原成源码并返回所在行数
...
def get_all_block_of_cfgs(flattened_cfg):
    ...
    return all_block
#将CFG中所有block以字典形式进行存储，key为block.id
...
def get_def_class_info(flattened_cfg):
    ...
    for key in flattened_cfg:
        ...
        if key == 'mod':  # 在主程序中，记录类的定义情况，注意，这里我们默认类只在主程序中定义
            for node in cfg.entryblock.statements:
                if isinstance(node, ast.ClassDef):
                    ...
                    for base in node.bases:
                        class_dict[node.name]['base'].append(astor.to_source(base))  # 加入类中的父类名称
                    for func in node.body:
                        if isinstance(func, ast.FunctionDef):
                            class_dict[node.name]['func'].append(func.name)  # 加入类中的函数名称

                    if '__init__' in class_dict[node.name]['func']:
                        class_dict[node.name]['is_init'] = True

        if len(flattened_cfg[key].function_args) == 0:  # 检测函数定义,归入函数定义中，建立其在类外引用的入口
            name = key.replace('mod.', '')
            if '_init_' in name.lower():
                name = name.replace(".__init__", '')
            def_dict[name] = cfg
            def_cfg_dict[name] =key# 子CFG的类外入口，当访问到这种语句时，可以作为入口进入CFG中（当直接实例化类时，自动进入__init__中
    return  def_dict, class_dict ,def_cfg_dict # 返回类的定义情况和函数的定义情况
...
class coustomChecker(BaseChecker):
    ...
    def CFG_Forward_traversal(self,block,level,index_orgin = None,end_block = None,end_index= None):
        ...
        for path in paths:
            ...
            for id in range(len(path)):
                if id == 0 and level == 0 and self.trace_type!="all_code":  # 当处于正向遍历起始block时，判断开始节点是否为block的第最后一个语句
                      # 是最后一个语句的话，进入下一block
                    if isinstance(path[id].statements[index], ast.ClassDef):#当你的开始标志位于类定义起始点时，默认其搜索范围为类定义
                        ...
                        if level == 0 and is_error:  # 某条路径中没有满足要求时，报错
                            ...
                            return None
                    elif isinstance(path[id].statements[index], ast.FunctionDef):#当你的开始标志位于函数定义起始点时，默认其搜索范围为函数定义
                    ...
                    if index == len(path[id].statements) - 1:
                        continue
                    index = index + 1  # 不是第一个语句的话，向上追溯
                else:  # 不是起始block时，从block中最后一个语句进行向上遍历
                    index = 0
    ...
    def CFG_Reverse_tracing(self,block,level,index_orgin = None,end_block = None,end_index= None):
        ...
        for path in paths:
            ...
            index = index_orgin
            for id in range(len(path)):
                if id == 0 and level == 0:  # 当处于反向追溯起始block时，判断开始节点是否为block的第一个语句
                    if isinstance(path[id].statements[index], ast.ClassDef):  # 当你的开始标志位于类定义起始点时
                        for class_func in path[id].statements[index].body:
                            if isinstance(class_func, ast.FunctionDef):
                                if class_func.name != "__init__":
                                    is_error_class = self.CFG_Forward_traversal(
    ...
    def visit_module(self, node):
        global isadd
        if len(self.start_sign)>0:#当checker要求上下文相关时
            start_node =None
            for curr_node in node.body:
                matching_sign = next(
                    (start_sign for start_sign in self.start_sign if start_sign in curr_node.as_string()), None)
                if matching_sign:#找到开始标记
                    if self.fg_type == "dfg":
                        # for child in curr_node.get_children():
                        start_node =  self.recursive_search(curr_node,matching_sign)
                        #     if (isinstance(child, astroid.Call) or isinstance(child, astroid.ClassDef)) and self.start_sign in child.as_string() :
                        #         start_node = child
                        if start_node ==None:
            ...
            if len(self.end_sign)>0:#当checker要求为区间追溯时
                end_node = None
                for curr_node in node.body:
        ...
        elif self.trace_type == "all_code":#当前选择的是寻找所有的代码，而不是寻找匹配的代码时：
...
def main(path_user,test_scripts):
    ...
    isfind =True
    isadd = False
    # test_scripts = [r"E:\checkers_of_silent_bugs_in_ml_project\test_example\18_view_test.py"]
    ...
    for script_path in test_scripts:
        ...
        flattened_cfg = cfg_builder.build_from_file(test_script ,
                                                    script_path,
                                                    flattened=True)
        # flattened_cfg_lineno_scopes = get_flattened_cfg_lineno_scopes(flattened_cfg)
        ...
        def_dict,class_dict,def_cfg_dict = get_def_class_info(flattened_cfg)#获取函数和类的定义信息
        code = read_python_file(script_path)

        build_and_draw_cfgs(flattened_cfg, graph)
        graph.render('flattened_cfg', view=False)
        print(f"Currently inspecting the documents ：{script_path}")
        Run([
            script_path,
